...
**Current target:** see `release-notes/0.2.3.md` (next release after published **0.2.2**).

### How publishing works

1. Set `<Version>` in `src/Stackroot.App/Stackroot.App.csproj` (must match the tag).
2. Write release notes in `release-notes/{Version}.md` — **not** in `0.2.2.md` after that version is already on GitHub.
